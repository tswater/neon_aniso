...
for sty in st_yr[rank::size]:
    ...
    for doy in doys:
        ...
        for t in range(n1m):
            ...
            if (np.sum(np.isnan(wp*wp))<.9*20*60):
        ...
        for t in range(n30m):
            ...
            combo=np.array([ld_v[t*20*60*30:(t+1)*20*60*30],ld_u[t*20*60*30:(t+1)*20*60*30],ld_w[t*20*60*30:(t+1)*20*60*30]])
            [rot_u,rot_v,rot_w]=np.dot(mat_rot,combo)
            rot_u=np.squeeze(np.array(rot_u[0,:]).T)
            rot_v=np.squeeze(np.array(rot_v[0,:]).T)
            rot_w=np.squeeze(np.array(rot_w[0,:]).T)
        ...
        if (dt0+timedelta(days=doy+1)).day==1:
            ...
            fp=h5py.File(out_dir+site+'/'+outname,'r+')
