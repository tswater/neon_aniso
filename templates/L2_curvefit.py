...
# %% [markdown]
# ## Load in the Data
# First load in the data to fit 

# %%
l2file=''
fp=h5py.File(l2file,'r') # start with open for readonly
fpsites=fp['main/data']['SITE'][:]
zL=fp['main/data']['zL'][:]
ani=fp['main/data']['ANI_YB'][:]
phi=get_phi(fp['main/data'],'UU')

# %% [markdown]
# ## How To Run a Full Case
...
params=['a0','a1'] # params to fit; in cbase, a(yb)=a0+a1(yb)**1+a2(yb)**2+...

p0=[2.55,0] # list of initial guesses for parameter values; one for each "None" in const

bnds = [.1,-5],[10,5]) # bounds for parameters. Form ([MIN1,MIN2,...],[MAX1,MAX2,...])
...
name='test_full' # name for your fit
...
# %%
#### Create Cfit Object
cf=Cfit(name,var,stab,'full',base,p0=p0,const=const,bnds=bnds,loss=loss,params=params)
...
# %%
#### Evaluate the fit
cf.eval_cfit(zL,ani,phi,fpsites)
compare_fits([cf]) # show the fit performance
...
name='test_2stage' # name for your fit
...
# %%
#### Create Cfit Object
cf=Cfit(name,var,stab,'2_stage',base,p0=p0,const=const,bnds=bnds,loss=loss,deg=deg,typ=typ)
