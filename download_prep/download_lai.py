...
for file in sitelist0:
    if (file in os.listdir(dwnld_dir+'lai')):
        if (len(os.listdir(dwnld_dir+'lai/'+file))>0):
            pass
        else:
...
for site in sitelist[rank::size]:
    ...
    try:
        subprocess.run('mkdir lai/'+site,shell=True)
    except:
        pass
    ...
    for i in range(dt):
        ...
        for ver in [1,2]:
            ...
            wstr="https://data.neonscience.org/api/v0/data/DP3.30012.00"+str(ver)+"/"
    ...
    subprocess.run('mv * ../../lai/'+site+'/',shell=True)
