...
for file in sitelist0:
    if (file in os.listdir(dwnld_dir+'albedo')):
        if (len(os.listdir(dwnld_dir+'albedo/'+file))>0):
            pass
        else:
...
for site in sitelist[rank::size]:
    ...
    try:
        subprocess.run('mkdir albedo/'+site,shell=True)
    except:
        pass
    ...
    for i in range(dt):
        ...
        for ver in [1,2]:
            ...
            wstr="https://data.neonscience.org/api/v0/data/DP3.30011.00"+str(ver)+"/"
    ...
    for folder in os.listdir('./'):
        ...
        for file in files:
            a=file['url']
            if ('.tif' in a) and ('error' not in a) and ('Error' not in a):
                dwnld_list.append(a)
    ...
    subprocess.run('mv * ../../albedo/'+site+'/',shell=True)
