...
for file in sitelist0:
    if (file in os.listdir(dwnld_dir+'trad')):
        if (len(os.listdir(dwnld_dir+'trad/'+file))>0):
            pass
        else:
...
for site in sitelist[rank::size]:
    ...
    try:
        subprocess.run('mkdir trad/'+site,shell=True)
    except:
        pass
    ...
    for i in range(dt):
        ...
        for ver in [1,2]:
            ...
            wstr="https://data.neonscience.org/api/v0/data/DP1.00005.00"+str(ver)+"/"
    ...
    for folder in os.listdir('./'):
        ...
        for file in files:
            a=file['url']
            if ('.csv' in a) and ('error' not in a) and ('Error' not in a):
                dwnld_list.append(a)
