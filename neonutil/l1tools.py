...
def add_profile_tqc(scl,ndir,dp4dir,dlt=None,addprof=True,addqaqc=True,ivars=None,\
    ...
    for site in sites:
            ...
            for i in range(len(lvltqc)-1):
                if addprof:
                    ...
                    ovar[v_long][v.upper()+str(i)]=\
                            nscale(time2,inp['t_'+v][i][:],inp[v][i][:],scl=scl,nearest=False,debug=debug)
                if addqaqc:
                    if debug:
                        print('addqaqc for '+v_long)
                    tmp=nscale(time2,inp['t_'+v][i][:],inp['q'+v][i][:],scl=scl,nearest=False,debug=debug)
        ...
        if 't' in vs:
            ...
            if addqaqc:
                tmp=nscale(time2,inp['t_ttop'][:],inp['qttop'][:],scl=scl,nearest=False,debug=debug)
                ovar['q'+v_long]=ovar['q'+v_long]+tmp[:]
                ovar['q'+v_long][ovar['q'+v_long]<.2]=0
                ovar['q'+v_long+'_upper']=ovar['q'+v_long+'_upper'][:]+tmp[:]
                ovar['q'+v_long+'_upper'][ovar['q'+v_long+'_upper']<.2]=0
...
def add_radiation(scl,ndir,idir,dlt=None,adddata=True,addqaqc=True,ivars=None,overwrite=False,sites=SITES,debug=False):
    ...
    for site in sites:
        ...
        # Interpolate Data
        if adddata:
            swin=nscale(time2,tm,d['inSWMean'],scl=scl,debug=debug)
            swout=nscale(time2,tm,d['outSWMean'],scl=scl,debug=debug)
            lwin=nscale(time2,tm,d['inLWMean'],scl=scl,debug=debug)
            lwout=nscale(time2,tm,d['outLWMean'],scl=scl,debug=debug)
            for v in ovar.keys():
                match v:
                    case 'SW_IN':
                        ovar[v]=swin
        ...
        if addqaqc:
            radq=nscale(time2,tm,d['inSWFinalQF'],scl=scl,debug=debug)
            radq=radq+nscale(time2,tm,d['outSWFinalQF'],scl=scl,debug=debug)
            radq=radq+nscale(time2,tm,d['inLWFinalQF'],scl=scl,debug=debug)
            radq=radq+nscale(time2,tm,d['outLWFinalQF'],scl=scl,debug=debug)
            ovar['qNETRAD']=radq
...
def add_ghflx(scl,ndir,idir,dlt=None,adddata=True,addqaqc=True,ivars=None,overwrite=False,sites=SITES):
    ...
    for site in sites:
        ...
        qgg[0,:]=nscale(time2,tm1,d1['finalQF'],scl=scl)
        qgg[1,:]=nscale(time2,tm2,d2['finalQF'],scl=scl)
        qgg[2,:]=nscale(time2,tm3,d3['finalQF'],scl=scl)

        if adddata:
            gg=np.zeros((3,len(time2)))
            gg[0,:]=nscale(time2,tm1,d1['SHFMean'],scl=scl)
            gg[1,:]=nscale(time2,tm2,d2['SHFMean'],scl=scl)
            gg[2,:]=nscale(time2,tm3,d3['SHFMean'],scl=scl)
            gg[gg<=-999]=float('nan')
    ...
    for site in sites:
        ...
        for var in ovar.keys():
            if 'scale_nearest' in basepath[var].keys():
                ...
                nearest=True
            ovar[var]=nscale(time2,tmp[var+'_time'],tmp[var],scl=scl,nearest=nearest)
...
def add_qaqc(scl,ndir,idir,dlt=None,ivars=None,qsci=False,overwrite=False,\
        confirm=False,sites=SITES,debug=False):
    ...
    for site in sites:
        if debug:
            dbg='::::::::::::::::DEBUG:::::::::::::::::::\n'
            dbg=dbg+'Loading QAQC information for '+site+'\n'
            print(dbg+':::::::::::::::::DEBUG::::::::::::::::::',flush=True)
        # Setup
        fpo=h5py.File(ndir+site+'_'+str(scl)+'m.h5','r+')
        time=fpo['TIME'][:]
        ...
        for file in filelist:
            ...
            for var in tmp.keys():
                try:
                    ...
                    print(e)
                    msg='The quality flag '+str(var)+' appears to be missing '+\
                        'for '+str(file)+'\nWould you like to return all NaN?'
                    if (debug and _confirm_user(msg)) or (not debug):
                        if var in ['qUVW','qU','qV','qW','qTHETA']:
                            n_=nqs
                        elif var in ['qQ','qC']:
                            n_=ncq
                        else:
                            n_=nqf
                        tmp[var].extend([-1]*n_)
                        print('returning all Nan for '+str(var)+' in '+\
                                str(file)+' to resolve exception... continuing')
                    else:
                        print('OKAY; will try to provide debug info: ')
                        try:
                            print('length of timeseries '+str(n_))
        ...
        for var in ovar.keys():
            ...
            if var in ['qsU','qsV','qsW','qsTHETA','qsUVW','qU','qV','qW','qTHETA','qUVW']:
                ...
                tmin=tmp['flxtime']
            ovar[var]=nscale(time2,tmin,tmp[var],scl=scl,debug=debug)
...
def add_pheno(scl,ndir,idir,dlt=None,ivars=None,overwrite=False,sites=SITES,debug=False):
    ...
    for site in sites:
        ...
        if 'SOLAR_ALTITUDE' in ovar.keys():
            ...
            for i in range(N):
                sangles[i,:]=nscale(time2,inp[i]['time'],inp[i]['solar_elev'],scl=scl,debug=debug,nearest=False)
        ...
        if ('GCC90_C' in ovar.keys())|('GCC90_D' in ovar.keys()):
            ...
            for i in range(Nd):
                dgcc[i,:]=nscale(time2,dinp[i]['time'],dinp[i]['gcc_90'],debug=debug,scl=scl,nearest=False)
            for i in range(Ne):
                egcc[i,:]=nscale(time2,einp[i]['time'],einp[i]['gcc_90'],debug=debug,scl=scl,nearest=False)
        ...
        if 'NDVI90' in ovar.keys():
            ...
            sangles=np.zeros((N,len(time)))
            for i in range(N):
                sangles[i,:]=nscale(time2,inp[i]['time'],inp[i]['ndvi_90'],nearest=False,scl=scl,debug=debug)
        ...
        out_to_h5(fpo,ovar,overwrite,desc)
